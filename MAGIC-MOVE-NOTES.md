...
- `svgs.spec.ts` covers every distinct SVG shape-morph code path (`animateRect`/`Circle`/`Line`/`Polyline`/`Polygon`/`Ellipse`/`Text`/`Path`) in `examples/svgs.qmd`, one sequence per shape type, forward and backward like everything else. `initSvgMagicMove` pre-inlines every magic-move slide's SVG (fetches each `img[data-src$=".svg"]`, swaps it for a real `<svg>`) in one sequential loop over the *whole document* before attaching its `slidechanged` listener, so `waitForSvgMagicMoveReady()` waits for `pie-chart` (the last SVG magic-move slide in the file) to be inlined as a proxy for "the whole page is ready" — triggering navigation before that finishes silently falls through to a plain slide change with no animation.
- `captureFrame()`/`captureFilmstrip()` take an optional `maxDiffPixelRatio` override (default comes from `filmstrip.ts`). `svgs.spec.ts` uses a higher one (8% vs. the 4% default): full-page screenshots of large, fast shape morphs move more pixels per ms than a small locator-scoped code block does, so the same real-time sampling drift shifts the diff ratio further — verified flaky at the default across repeated runs, stable at 8%.
- **Known finding, not a testing artifact:** `examples/svgs.qmd`'s "Bar Chart" and "Pie Chart" slides have no `## Break` between them, so `groupConsecutiveSvgSlides` groups them into one sequence by accident — they're unrelated chart types, not a before/after pair. Its *mid-flight* frames are genuinely non-deterministic (verified 8-14% diff ratios across repeated runs, while the settled start/end states were stable every time), and watching the replay video shows why: rects crumple into unstable dart/triangle shapes before settling into the pie wedges. `runSvgSequence()`'s `captureMidFrames` param lets this one test skip mid-flight sampling and pixel-diff only the settled ends, but the underlying instability is in `magic-move.js`'s shape-interpolation for a mismatch this large (or in the example's missing `## Break`) — worth a maintainer's attention independent of this test suite.
