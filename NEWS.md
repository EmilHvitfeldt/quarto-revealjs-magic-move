...
- Slide-based magic-move's `.sourceCode` height transition had the same fixed-`[0, duration]`-window bug as the div-based wrapper above and is now fixed the same way, syncing to the `move` schedule (falling back to exit/enter) instead of always running immediately for a flat `duration`.
