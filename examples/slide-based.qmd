---
title: Slide-Based Magic Move Demo
format: revealjs
revealjs-plugins:
  - magic-move
---

## Introduction

This demo shows slide-based magic move where consecutive slides with the `.magic-move` class animate between each other.

## Step 1 {.magic-move}

```r
Year.Release <- game$Year.Release
counts <- data.frame(table(Year.Release))
```

## Step 2 {.magic-move}

```r
Year.Release <- game$Year.Release
counts <- data.frame(
  table(Year.Release)
)
```

## Step 3 {.magic-move}

```r
Year.Release <- game$Year.Release
counts <- data.frame(
  table(Year.Release)
)
p <- game %>%
  select(Year.Release, Global.Sales)
```

## Step 4 {.magic-move}

```r
Year.Release <- game$Year.Release
counts <- data.frame(
  table(Year.Release)
)
p <- game %>%
  select(Year.Release, Global.Sales) %>%
  group_by(Year.Release) %>%
  summarise(Total.Sales = sum(Global.Sales))
```

## Break

This is a normal slide without magic-move.

It breaks the sequence.

## Another sequence {.magic-move}

```javascript
const x = 1
```

## Final {.magic-move}

```javascript
const x = 1
const z = x + y
const y = 2
```

## Continued {.magic-move}

```javascript
const x = 1
const y = 2
```



## Staggered exit step 1 {.magic-move delay-exit=0 stagger=0.3 delay-move=1.3 .smaller}

```r
a <- 1
b <- 2
c <- 3
d <- 4
```

## Staggered exit step 2 {.magic-move delay-exit=0 stagger=0.3 delay-move=1.3 .smaller}

```r
a <- 1
d <- 4
```

## End

That's all folks!
